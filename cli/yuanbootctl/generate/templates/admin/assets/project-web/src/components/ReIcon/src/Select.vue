...
function setVal() {
  ...
  const prefix = inputValue.value.substring(
    0,
    inputValue.value.indexOf(":") + 1
  );
  if (!copyIconList[prefix]) return;
  currentActiveType.value = prefix;
...
function onBeforeEnter() {
  ...
  setVal();
  const currentIconList = copyIconList[currentActiveType.value] || [];
  const curIconIndex = currentIconList.findIndex(i => i === icon.value);
