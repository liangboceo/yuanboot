        ...
        case "push":
          {
            ...
            const tagName = tagVal.name;
            const tagHasExits = this.multiTags.some(tag => {
              const sameMenu =
                (tagName && tag.name === tagName) || tag.path === tagPath;
              if (!sameMenu) return false;

              const hasQueryOrParams =
                Object.keys(tagVal?.query ?? {}).length > 0 ||
                Object.keys(tagVal?.params ?? {}).length > 0 ||
                Object.keys(tag?.query ?? {}).length > 0 ||
                Object.keys(tag?.params ?? {}).length > 0;

              if (!hasQueryOrParams) return true;
