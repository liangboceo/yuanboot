...
var Project = projects.NewEmptyProject("microservice", "Complete Microservice Application").With(func(root *projects.ProjectItem) {
	...
	root.AddDir("internal").AddDir("service").AddFileWithContent("dbs_service.go", DbsService_Tel)
	root.AddDir("internal").AddDir("service").AddFileWithContent("cache_service.go", CacheService_Tel)
	...
	root.AddDir("config").AddFileWithContent("log.yml", Log_Tel)
	root.AddFileWithContent("go.mod", Mod_Tel)
	root.AddFileWithContent(".gitignore", Gitignore_tpl)
